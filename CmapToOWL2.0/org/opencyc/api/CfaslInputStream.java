 ...
 * 
 * @version $Id: CfaslInputStream.java 128075 2009-06-19 23:22:28Z tbrussea $
...
protected void initializeOpcodeDescriptions() {
    ...
    cfaslOpcodeDescriptions.put(new Integer(CFASL_LEGACY_GUID), 
                                "CFASL_LEGACY_GUID");
  ...
  public Object readObject()
    ...
    if (trace == API_TRACE_DETAILED) {
      try {
        ...
        Log.current.println("readObject = " + safeToString.invoke(o) + " (" + o.getClass() + ")");
      }
       catch (Exception e) {
  ...
  public Guid readLegacyGuid()
    ...
    if (trace == API_TRACE_DETAILED) {
      Log.current.println("readLegacyGuid: " + guid);
    }
...
public CycList readCons()
    ...
    if (trace == API_TRACE_DETAILED) {
        try {
          ...
          Log.current.println("readCons.cdrObject: " + safeToString.invoke(cdrObject));
        } catch (Exception e) {
